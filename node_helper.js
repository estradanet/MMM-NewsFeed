...
module.exports = NodeHelper.create({
  ...
  initialize: async function () {
    ...
    console.log("[FEED] MMM-NewsFeed is now initialized!")
  ...
  getInfos: async function () {
    ...
    log("Items older than 24h removed. Remaining:", this.RSS.length);
  ...
  getImage(item) {
    ...
    if (item.description && item.description.match(/<img[^>"']*((("[^"]*")|('[^']*'))[^"'>]*)*>/g)) {
